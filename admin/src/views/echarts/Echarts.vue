...
<template>
  <section>
    <el-row :gutter="5">
      <el-col :sx="24" :sm="12" :lg="12" :md="12">
        <!-- 柱状图 -->
...
import * as tree_data from "../../utils/tree.data.js";
...
export default {
  ...
  methods: {
    ...
    province() {
      ...
      var treeData = transData(data, "value", "sj", "children");
      /*
        function transData(a, idStr, pidStr, childrenStr) {
          ...
          // hash={01: {name: "广东", value: "01", sj: "-"}}
          for (; j < len; j++) {
            var aVal = a[j], //data中的每一项数据
              hashVP = hash[aVal[pid]]; //aVal[pid]拿到每一项数据的sj
            if (hashVP) {
      ...
      */
      function transData(data, idStr, pidStr, childrenStr) {
        var temp = [],
          obj = {},
          id = idStr,
          pid = pidStr,
        ...
        for (var i = 0; i < len; i++) {
          var item = data[i];
          obj[item[id]] = item;
          var hashVP = obj[item[pid]]; //item[pid]拿到每一项数据的sj
          if (hashVP) {
            !hashVP[children] && (hashVP[children] = []);
      ...
      // console.log(999, treeData);
      // return;
      var myChart = this.$echarts.init(document.getElementById("province"));
      var option = {
        title: {
          text: "Tree节点遍历",
          textStyle: {
        ...
        series: [
          {
            ...
            label: {
              ...
              expendAndCollapse: true,
              initialTreeDepth: 2 //展示层级数
            }
          }
        ]
      };
