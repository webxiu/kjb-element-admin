	...
	<div class="content-page uk-body">
		...
		<div class="calendar">
			...
			<table class="sign_tab" border="0px" cellpadding="0px" cellspacing="0px">
				...
				<tbody>
					<tr v-for="(item,index) in dateArr" :key="index" v-if="contains(item)">
						<td v-for="(data,i) in item" :key="i" :class="{ 'cur_day': data == date, 'ui-state-default': true }" v-if="isCheck(data)">
						</td>
...
<style scoped>
	...
	.calendar{width:600px;height:600px;margin:50px 0px 20px 0px;color:#555;position: relative;}
	...
	.calendar button.month-less {left: 20px;background: url(../../assets/img/left-icon.png) no-repeat left -60px;}
	.calendar button.month-add {right: 20px;background: url(../../assets/img/right-icon.png) no-repeat left -60px;}
	.calendar .sign_tab{width: 100%;border-collapse: collapse;border: 1px solid #e8e8e8;border-top: 0;table-layout: fixed;}
	.calendar .sign_tab th{text-align: center;height: 60px;    line-height: 60px;font-weight: 700;}
	.calendar .sign_tab td{border: 1px solid #e8e8e8;height: 60px;    line-height: 60px;text-align: center;font-size: 22px;font-family: arial;}
	.calendar .sign_tab td.over{background-color: #fff;border-left: 0;border-right: 0;}
	.calendar .ui-state-default{background: url(../../assets/img/checkin.png) no-repeat center center;}
